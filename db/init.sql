...
CREATE TABLE IF NOT EXISTS stats (
    ...
    tov             NUMERIC(5,2),
    fg_pct          NUMERIC(5,3),
    fg3_pct         NUMERIC(5,3) DEFAULT 0.0,
    ft_pct          NUMERIC(5,3) DEFAULT 0.0,
...
SELECT p.player_id,
    ...
    st.age, st.gp, st.min, st.pts, st.reb, st.ast, st.stl, st.blk, st.tov,
    st.fg_pct, st.fg3_pct, st.ft_pct, st.net_rating, st.usg_pct, st.ast_pct,
    st.ast_to, st.per, f.career_year, f.pts_delta, f.reb_delta, f.ast_delta,
    COALESCE(f.ovr_prev, r_prev.ovr_rating, r.ovr_rating) AS ovr_prev,
    f.ovr_delta,
    CASE
        WHEN st.season_year <= 2023 THEN 'train'
        WHEN st.season_year = ANY (ARRAY[2024, 2025]) THEN 'test'
        ELSE 'predict'
    END AS split
FROM stats st
...
LEFT JOIN ratings r_prev ON r_prev.player_id = st.player_id AND r_prev.season_year = st.season_year - 1
